...
app.post('/api/entries/non-registered', async (req, res) => {
  ...
  try {
    ...
    if (existingEntry) {
      return res.status(400).json({ message: "כבר קיימת כניסה היום למספר זיהוי זה" });
    }
...
app.post('/api/departments', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (req.admin.role === 'gymAdmin' && req.admin.baseId.toString() !== baseId) {
...
app.post('/api/subDepartments', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (req.admin.role === 'gymAdmin' && req.admin.baseId.toString() !== department.baseId.toString()) {
...
app.put('/api/trainees/:id/medical-approval', authMiddleware, async (req, res) => {
  const { 
    approved, 
    expirationDate, 
    medicalFormScore, 
  ...
  try {
    ...
    trainee.medicalApproval = {
      ...
      expirationDate: expirationDate || (approved ? new Date(Date.now() + 365 * 24 * 60 * 60 * 1000) : null)
    };
...
app.post('/api/entries', async (req, res) => {
  const { 
    traineeId, entryDate, entryTime, 
    traineeFullName, traineePersonalId, 
    departmentId, subDepartmentId, baseId, status // Add subDepartmentId here
  } = req.body;
  ...
  try {
    ...
    if (existingEntry && existingEntry.status === EntryStatus.SUCCESS) {
      return res.status(400).json({ message: "המשתמש כבר נכנס היום, הצהרת בריאות בתוקף" });
    } 
